...
namespace ProjectPaula.Model.CalendarExport
{
    public class ScheduleExporter
    {
        public static string ExportSchedule(Schedule schedule, string username)
        {
            ...
            var dates = courses.SelectMany(c => c.Dates.Select(d => new CalendarEvent() { StartTime = d.From, EndTime = d.To, Location = d.Room, Name = c.Name, Organizer = d.Instructor })).ToList();
            dates.AddRange(courses.SelectMany(c => c.ExamDates.Select(d => new CalendarEvent() { StartTime = d.From, EndTime = d.To, Name = d.Description, Location = "", Organizer = d.Instructor })));
